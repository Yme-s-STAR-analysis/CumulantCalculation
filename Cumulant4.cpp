...
int main(int argc, char** argv) {
    ...
    const int nCums = 27; // cums for cumulant and correlation function (orders) 
    ...
    const char* cumNames[nCums] = {
        ...
        "R21r", "R2sr", "R31r", "R32r", "R3sr", "R42r", // 24
        "k21r", "k31r", "k41r" // 27
    };
    ...
    for (int i=0; i<nType; i++) {
        for (int j=0; j<nCums; j++) {
            if (i == 2) { // skip kappa for net proton
                if ((j >= 11 && j <= 17) || j >= 24) { continue; } 
            } else { // skip ppb for proton and antiproton
                if (j == 1 || j == 6 || j == 9 || j == 19 || j == 22) { continue; }
            }
            if (j <= 17) { // directly apply CBWC
                tfin->GetObject(
                    Form("%s%s", typeNames[i], cumNames[j]), 
                    sCums[i][j]
                );
            } else if (j <= 23) { // cumulant ratios
                tfin->GetObject(
                ...
                tfin->GetObject(
                    Form("%s%s", typeNames[i], cumNames[j-9]), 
                    sCums[i][j]
                );
            }
    ...
    for (int i=0; i<nType; i++) { // i for proton types
        for (int j=0; j<nCums; j++) { // j for cumulant orders
            ...
            for (int k=LowMultCut; k<=MaxMult; k++) { // here k for refmult3
                ...
                if (j <= 17) { // for those cumulants and ratios, directly apply CBWC
            ...
            for (int k=0; k<nCent; k++) {
                ...
                if (j <= 17) {
                    ...
                    vSum[i][k] = *(tgs[i][j-12]->GetY()+k) / *(tgs[i][11]->GetY()+k); // k(2~4) / k1
                }
                // calculate errors
